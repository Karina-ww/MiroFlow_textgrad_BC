...
class GPT5OpenAIClient(LLMProviderClientBase):
    ...
    async def _create_message(
        ...
        try:
            ...
            assert self.model_name in ["gpt-5-2025-08-07", "gpt-5", "gpt-5.1"]
            ...
            # Only add reasoning_effort for models that support it (o1, o3 series)
            # Skip for gpt-5 to avoid API compatibility issues with some endpoints
            # if self.model_name and ("o1" in self.model_name.lower() or "o3" in self.model_name.lower()):
    ...
    async def _create_completion(self, params: Dict[str, Any], is_async: bool):
        ...
        # Force stream=False to be explicit
        params["stream"] = False
        ...
        # Sanity check: ensure we got a proper object, not a string
        if isinstance(response, str):
            logger.error(f"API returned raw string instead of parsed object. This indicates the endpoint returned streaming format despite stream=False. Response preview: {response[:500]}")
            raise Exception(
                "API endpoint returned streaming format (SSE text) despite stream=False. "
                "This is likely a bug in the API endpoint under concurrent load. "
                "Consider reducing max_concurrent or contacting API provider."
            )
