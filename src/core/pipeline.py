...
async def execute_task_pipeline(
    ...
    try:
        ...
        if hasattr(cfg.main_agent, "llm") and cfg.main_agent.llm is not None:
            ...
            # Attach token_counter to main agent LLM client
            if token_counter:
                main_agent_llm_client.token_counter = token_counter
        else:
            raise ValueError(
        ...
        if cfg.sub_agents is not None and cfg.sub_agents:
            ...
            if hasattr(first_sub_agent, "llm") and first_sub_agent.llm is not None:
                ...
                # Attach token_counter to sub agent LLM client
                if token_counter:
                    sub_agent_llm_client.token_counter = token_counter
            else:
                raise ValueError(
        ...
        orchestrator = Orchestrator(
            ...
            sub_agent_llm_client=sub_agent_llm_client, # <class 'src.llm.providers.gpt5_openai_client.GPT5OpenAIClient'>
        ...
        final_answer, final_boxed_answer = await orchestrator.run_main_agent(
            task_description=task_description, # 'What is the final numeric output from the attached Python code?'
            task_file_name=task_file_name, # 'data/gaia-val/f918266a-b3e0-4914-865d-4faa564f1aef.py'
            task_id=task_id, # 'f918266a-b3e0-4914-865d-4faa564f1aef'
        )
