...
class Orchestrator:
    ...
    async def _handle_llm_call_with_logging(
        ...
        try:
            ...
            ) # 像是analysis -- 分析现在需要做什么，Tool choice for this step
        ...
        while turn_count < max_turns:
            ...
            for call in tool_calls[0][:max_tool_calls]:
                ...
                try:
                    ...
                    ].execute_tool_call(server_name, tool_name, arguments) # 'tool-code' 'create_sandbox' {}
            ...
            try:
                hint_content = await extract_hints(
                    ...
                    self.cfg.main_agent.input_process.get(
                        "hint_llm_base_url", "https://yibuapi.com/v1"
                    ),
                )
                hint_notes = (
            ...
            for call in tool_calls[0][:max_tool_calls]:
                ...
                try:
                    if server_name.startswith("agent-"):
                        sub_agent_result = await self.run_sub_agent( ### WWW: sub-agent!
