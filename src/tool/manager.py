                ...
                if isinstance(server_params, StdioServerParameters):
                    async with stdio_client(
                        ...
                        async with ClientSession(
                            ...
                            await session.initialize()
                            try:
                                tool_result = await session.call_tool(  # WWW: 在这里call
                                    tool_name, arguments=arguments # read_file {'uri': 'file:///data/wengtengjin/wtj_works/MiroFlow/data/gaia-val/f918266a-b3e0-4914-865d-4faa564f1aef.py'}
                                )
