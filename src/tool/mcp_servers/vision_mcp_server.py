...
GEMINI_BASE_URL = os.environ.get("GEMINI_BASE_URL", "https://yibuapi.com/v1")
GEMINI_MODEL_NAME = os.environ.get("GEMINI_MODEL_NAME", "gemini-2.5-flash-image-preview")
...
async def call_gemini_vision(image_path_or_url: str, question: str) -> str:
    """Call Gemini vision API using OpenAI-compatible interface."""
    try:
        if os.path.exists(image_path_or_url):  # Check if the file exists locally
            with open(image_path_or_url, "rb") as image_file:
                image_data = base64.b64encode(image_file.read()).decode("utf-8")
        ...
        return f"[ERROR]: Failed to call Gemini vision: {e}"
    ...
    if provide_transcribe:
        ...
        while retry_count <= max_retry:
            try:
                ...
                result = transcribe_response.choices[0].message.content

                # Check if response is None or empty after stripping
                if result is None or result.strip() == "":
                    raise Exception("Response text is None or empty")

                transcribe_content = "Transcription:\n\n" + result + "\n\n"
                break
            except Exception as e:
